...
extension View {
    func backgrondBlur(radius: CGFloat = 3, opaque: Bool = false) -> some View {
        self
            .background(
...
extension View {
    func innerShadow<S: Shape, SS: ShapeStyle>(shape: S, color: SS, lineWight: CGFloat = 1, offsetX: CGFloat = 0, offsetY: CGFloat = 0, blur: CGFloat = 4, blendMode: BlendMode = .normal, oposity: Double = 1) -> some View {
        return self
            .overlay {
                //MARK: Bottom Sheet Inner Shadow (Border)
                shape
                    .stroke(color, lineWidth: lineWight)
                    .blendMode(blendMode)
